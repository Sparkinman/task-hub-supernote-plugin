...
function Build-AndroidApk {
    ...
    try {
        ...
        if (Test-Path $gradlewPath) {
            ...
            # NOTE (local fix): Start-Process does not inherit Set-Location, so it
            # ran in the project root where gradlew.bat does not exist and failed
            # with "not recognized". Pass the full path and an explicit
            # -WorkingDirectory instead.
            $quotedGradlew = '"' + $gradlewPath + '"'
            $process = Start-Process -FilePath 'cmd.exe' -ArgumentList '/c', $quotedGradlew, 'buildCustomApkDebug' -WorkingDirectory $androidDir -Wait -PassThru -NoNewWindow
            $buildResult = $process.ExitCode
        }
        elseif (Get-Command 'gradle' -ErrorAction SilentlyContinue) {
            Write-ColorOutput 'Using gradle to execute buildCustomApkDebug task...' 'Green'
            $process = Start-Process -FilePath 'gradle' -ArgumentList 'buildCustomApkDebug' -WorkingDirectory $androidDir -Wait -PassThru -NoNewWindow
            $buildResult = $process.ExitCode
        }
        else {
